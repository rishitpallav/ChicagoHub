...
@Component({
  selector: 'heatmap-realtime',
  templateUrl: './heat-map-realtime.component.html',
  styleUrls: ['./heat-map-realtime.component.scss']
})


export class HeatMapRealtimeComponent implements OnInit{
...
//////////////////////////////////////////////////////////////////////////////
  ...
  changeTimeRangeSelected(data){
    ...
    console.log('newTimeHourStart', this.newTimeHourStart);
    console.log('newTimeHourEnd', this.newTimeHourEnd);
    // console.log('new Date()', new Date().setHours(12,0,0));
    // console.log('new Date()', new Date().setHours(13,0,0));
    this.clearHeatMap();
  ...
  getDivvyStationsStatus(timeRangeSelection){
    ...
    currentTimeStart.setHours(this.newTimeHourStart,0,0,0);
    // console.log('currentTimStart', currentTimeStart);

    var currentTimeEnd= new Date();
    currentTimeEnd.setHours(this.newTimeHourEnd,0,0,0);
    // console.log('currentTimeEnd', currentTimeEnd);

    var timeDiff = this.newTimeHourEnd - this.newTimeHourStart;
    ...
    if (this.timeRangeSelected == this.PAST_HOUR){
      ...
      startTimeForDataCollection = new Date(currentTimeEnd.getTime()- timeDiff * 60 * 60 * 1000);
      ...
      startTimeForDataCollection = new Date(currentTimeEnd.getTime() - 24* 60 * 60 * 1000);
      currentTimeForDataCollection = new Date(startTimeForDataCollection.getTime()+this.timeOffset);
    } 

    else if (this.timeRangeSelected == this.PAST_7_DAYS){
      this.noOfDivvyDataSamplesRequested = this.FRAMES_PER_WEEK;
      startTimeForDataCollection = new Date(currentTimeEnd.getTime() - 7*24* 60 * 60 * 1000);
      currentTimeForDataCollection = new Date(startTimeForDataCollection.getTime()+this.timeOffset);
    }
    ...
    if(this.noOfDivvyDataSamplesProcessed < this.noOfDivvyDataSamplesRequested){
      // console.log('timerangeselected ln290', this.timeRangeSelected);
      // console.log('timerangeselection ln290', timeRangeSelection);
      this.placesService.get_all_divvy_stations_data_range(this.timeRangeSelected, timeRangeSelection, currentTimeEnd, timeDiff).subscribe((data: Station[]) => {
        ...
        // So, current time is 
        if (this.timeRangeSelected == this.PAST_HOUR){
          this.timeOffset = (this.noOfDivvyDataSamplesProcessed) * (60 * 1000 * 2 * timeDiff);
        } else 
  ...
  plot_availableDocksInDivvyStations_on_heatMap(availableDocksInDivvyStations){
    ...
    //console.log(availableDocksInDivvyStations);

    // var newDocks=[];
    // var x = 0;
    // for(let i = 0; i < availableDocksInDivvyStations.length; i++) {
    //   var availableDocks = availableDocksInDivvyStations[i].availableDocks;
    //   var availableBikes = availableDocksInDivvyStations[i].availableBikes;
    //   // console.log('availableBikes', availableBikes);
    //   // console.log('availableDocks', availableDocks);
    //   if(availableBikes/(availableBikes + availableDocks) > 0.5 ) {
    //     this.stations[x] = availableDocksInDivvyStations[i];
    //     x++;
    //   }
    // }
