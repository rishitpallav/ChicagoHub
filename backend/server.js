...
router.route("/stations/fetch_divvy_stations_data_range").post((req, res) => {
  get_all_divvy_stations_log_range(
    req.body.timeRange,
    req.body.newTimeRangeSelection,
    req.body.currentTimeStart,
    req.body.timeDiff
  ).then(function (response) {
    var hits = response;
    var finalResponse = [];
    var x = 0;
    for (var i = 0; i < all_docks_found.length; i++) {
      var availableBikes = all_docks_found[i].availableBikes;
      var availableDocks = all_docks_found[i].availableDocks;
      if (availableBikes / (availableBikes + availableDocks) > 0.9) {
        finalResponse[x] = all_docks_found[i];
        x++;
      }
    }
    all_docks_found = [];
...
async function get_all_divvy_stations_log_range(
  timeRange,
  newTimeRangeSelection,
  currentTimeStart,
  timeDiff
) {
  var start_datetime_var = currentTimeStart;
  var scrollVal;
  ...
  if (timeRange.includes(PAST_HOUR)) {
    ...
    if (!isBeginningOfTimeRangeSet) {
      ...
      go_back_in_time_var = start_datetime_var.getHours() - timeDiff;
      ...
      var go_forward_in_time_var_2 =
        time_stamp_var_4.getMinutes() + 2 * timeDiff;
      time_stamp_var_4.setMinutes(go_forward_in_time_var_2);
  ...
  if (timeRange.includes(PAST_HOUR)) {
    ...
    diffMinutes = time_stamp_var_4.getMinutes() + 2 * timeDiff;
    time_stamp_var_4.setMinutes(diffMinutes);
